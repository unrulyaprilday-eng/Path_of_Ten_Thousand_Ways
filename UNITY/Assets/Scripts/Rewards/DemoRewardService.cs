...
namespace PathOfTenThousandWays.Demo.Rewards
{
    ...
    public sealed class DemoRouteRewardService
    {
        ...
        private static List<DemoReward> CreateOpeningChoices(DemoSwordStyle focusStyle)
        {
            ...
            {
                CreateRouteReward(
                    "route_opening_wanjian",
                    focusStyle,
                    DemoQuality.Earth,
                    "锋",
                    "开局历练",
                    "先试锋，再补件",
                    "先打一场试锋斗法，再拿第一批补件，让这一局先看清主脉，再决定后面怎么滚起来。",
                    new DemoMapNode(1, DemoNodeType.Battle, "云海试锋"),
                    new DemoMapNode(1, DemoNodeType.Reward, "起手补件"),
                    new DemoMapNode(2, DemoNodeType.RouteChoice, "第二层路口")),
                CreateRouteReward(
                    "route_opening_thunder",
                    focusStyle,
                    DemoQuality.Heaven,
                    "压",
                    "开局历练",
                    "先压路，再抢势",
                    "先走更高压的一段试锋路，把回报提早拿到手，适合想更快把节奏压高的开局。",
                    new DemoMapNode(1, DemoNodeType.Battle, "险路试锋"),
                    new DemoMapNode(1, DemoNodeType.Reward, "高压补件"),
                    new DemoMapNode(2, DemoNodeType.RouteChoice, "第二层路口")),
                CreateRouteReward(
                    "route_opening_blood",
                    focusStyle,
                    ...
                    "开局历练",
                    "先藏手，再探秘",
                    "先探一段更偏搜寻与试探的路，把第一轮补强留给后面的收束，适合想多看一眼缺口的开局。",
                    new DemoMapNode(1, DemoNodeType.Battle, "秘路试锋"),
                    new DemoMapNode(1, DemoNodeType.Reward, "搜寻所得"),
                    new DemoMapNode(2, DemoNodeType.RouteChoice, "第二层路口"))
            };
        }
